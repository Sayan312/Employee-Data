...
UPDATE EMPLOYEE SET FULL_NAME = CONCAT_WS(" ",FNAME,MINIT,LNAME);

-- EMPLOYEES WITH MORE THAN 1 DEPENDENT
SELECT E.FULL_NAME , COUNT(D.DEPENDENT_NAME) AS NUM_OF_DEPENDENTS FROM EMPLOYEE E JOIN DEPENDENT D ON E.SSN=D.ESSN GROUP BY E.FULL_NAME HAVING COUNT(D.DEPENDENT_NAME)>1;

-- EMPLOYEES WITH NO DEPENDENT
SELECT E.FULL_NAME , D.DEPENDENT_NAME AS NUM_OF_DEPENDENTS FROM EMPLOYEE E LEFT JOIN DEPENDENT D ON E.SSN=D.ESSN WHERE D.DEPENDENT_NAME IS NULL  ; 

-- MANAGERS 
SELECT E1.FULL_NAME AS EMPLOYEE , E2.FULL_NAME AS MANAGERS FROM EMPLOYEE E1 JOIN EMPLOYEE E2 ON E1.SUPERSSN=E2.SSN ;

-- EMPLOYEES WHO DOES NOT HAVE MANAGERS
SELECT FULL_NAME FROM EMPLOYEE WHERE SUPERSSN IS NULL;

-- EMPLOYEES WHO ARE NOT MANAGERS
SELECT * FROM EMPLOYEE WHERE SSN NOT IN (SELECT DISTINCT(SUPERSSN) FROM EMPLOYEE WHERE SUPERSSN IS NOT NULL); 

-- AVERAGE SALARY ON THE BASIS OF GENDER IN EACH DEPARTMENT
SELECT DISTINCT(DNO),SEX,AVG(SALARY)OVER(PARTITION BY DNO) AS AVG_SAL FROM EMPLOYEE ;

-- HIGHEST SALARY IN EACH DEPARTMENT
SELECT DISTINCT(DNO),MAX(SALARY)OVER(PARTITION BY DNO) AS MAX_SALARY FROM EMPLOYEE;

-- DISPLAY THE LEVEL OF SALARY
SELECT FULL_NAME , SSN , SALARY, 
CASE WHEN SALARY > (SELECT AVG(SALARY) FROM EMPLOYEE) THEN "Good Salary"
WHEN SALARY = (SELECT AVG(SALARY) FROM EMPLOYEE) THEN "Average Salary"
WHEN SALARY < (SELECT AVG(SALARY) FROM EMPLOYEE) THEN "Less Salary"
END AS Salary_Report
FROM EMPLOYEE; 

-- DISPLAY EMPLOYEE NAME WHERE "Alice as dependent" AS A CRITERIA
SELECT E.FULL_NAME AS FULL_NAME , E.SALARY AS SALARY , "Alice as dependent" AS CRITERIA FROM EMPLOYEE E JOIN DEPENDENT D WHERE E.SSN=D.ESSN;

-- EMPLOYEES WHO HAVE WORKED ON MORE THAN ONE PROJECT
SELECT E.FULL_NAME AS NAME , COUNT(W.PNO) AS NUMER_OF_PROJECTS FROM EMPLOYEE E JOIN WORKS_ON W ON E.SSN=W.ESSN GROUP BY E.FULL_NAME HAVING COUNT(W.PNO)>1;

-- MAXIMUM SALARY IN EACH DEPARTMENT
SELECT * FROM EMPLOYEE WHERE (DNO ,SALARY) IN (SELECT DNO,MAX(SALARY) FROM EMPLOYEE GROUP BY DNO);

-- DISPLAY SALARY , AVERAGE SALARY FOR ALL EMPLOYEES AS PER THEIR DEPARTMENT
SELECT FULL_NAME,SALARY,DNO,(SELECT AVG(SALARY) FROM EMPLOYEE) AS AVG_SALARY FROM EMPLOYEE;

-- MOVING AVERAGE OF SALARY
SELECT FULL_NAME,AVG(SALARY)OVER(ORDER BY BDATE ROWS BETWEEN 1 PRECEDING AND 1 FOLLOWING) AS MOV_AVG FROM EMPLOYEE;

-- LAG AND LEAD VALUE
SELECT FULL_NAME , LAG(SALARY,1,0)OVER(ORDER BY BDATE) AS LAG_SAL FROM EMPLOYEE;

SELECT FULL_NAME , LEAD(SALARY,1,0)OVER(ORDER BY BDATE) AS LEAD_SAL FROM EMPLOYEE;

-- LIST OF DNO WHERE TOTAL SALARY IS GREATER THAN AVGERAGE SALARY (USING CTE)
WITH DNO_TOT_SAL(DNO,SUM_SAL_DNO) AS (SELECT DNO,SUM(SALARY) AS SUM_SAL_DNO FROM EMPLOYEE GROUP BY DNO),
DNO_AVG_SAL(AVG_SAL_DNO) AS (SELECT AVG(DNO_TOT_SAL.SUM_SAL_DNO) AS AVG_SAL_DNO FROM DNO_TOT_SAL)
SELECT * FROM DNO_TOT_SAL TB1 JOIN DNO_AVG_SAL TB2 WHERE TB1.SUM_SAL_DNO>TB2.AVG_SAL_DNO;
